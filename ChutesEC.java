...
/**
 * The ChutesEC program implements an application that plays a Chutes and
 * Ladder game with 2 to 6 players. It uses a spin of 6 possibilities ranging
 * from 1 to 6. When a ladder is found at a certain location, player's
 * location is incremented by some offset. If chute is found, player's
 * location is decremented by certain offset. The game ends when a player
 * reaches location 100. The game also prints the unique winners in the last
 * 10 games to the console and stores the winner list in the file named
 * winners.txt
 *
 * @author  Ashlesha
 * @version 1.0
...
public class ChutesEC {
    ...
    // Declaring Constant for hyphen formatting
    static final String HYPHENS = "------------------------------";

    // Declaring Array which stores names of the players
    static String[] playerNames;

    // Declaring Parallel array which stores location of each player
    static int[] playerLocations;

    // Declaring Array to store chutes ladders offset mapping
    static int[] chutesLadders;

    // Declaring Max size of the chutes ladder board
    static final int MAX_SIZE = 100;

    // Declaring Min range for spin
    static final int SPIN_MIN = 1;

    // Declaring Max range for spin
    static final int SPIN_MAX = 6;

    // Declaring random to generate spin outcome
    static Random random = new Random();

    // Declaring Constant for file name
    static final String FILE_NAME = "/home/fac/ohsh/submit/20rq5001/files/" +
            "p3input.txt";

    // Declaring Constant for winner list file name
    static final String WINNER_FILE_NAME = "winners.txt";

    // Declaring Constant for maximum winners
    static final int MAX_WINNERS = 10;

    // Declaring Array of winners
    static String[] winners;

    // Declaring Index to keep track of index of last winner
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        // Creating a winners list file.
        // Maintaining a list of 10 winners. New winners are added at the
        // bottom.
        // When file becomes full, top most winner is removed.
     ...
     *
     * @param playerName Compares the winner name with already existing
     * winners
     * @return boolean True if player is in last 10 winner list, false
     * otherwise
     */
     ...
     * not found
     */
    public static void writeWinnersListToFile(File winnerFile)
                                                        throws IOException
    {
    ...
    public static void setChutesLadders(Scanner keyboard) throws IOException
    {
        ...
        while(keyboard.hasNext())
        {
            ...
            // Creating mapping between chutes/ladders and offset
     ...
     * @return  The string representing player name at index playerNumber.
     */
    ...
    /**
     * Returns the player location of player at given index in
     * playerLocations[].
     *
    ...
    /**
     * Sets the player location of player at given index in the
     * playerLocations[]
     *
     * @param playerNumber playerLocations array index representing the player.
    ...
    public static int playGame()
    {
        ...
        do
        {
            ...
            System.out.println(playerName +
                              ", it's your turn. You are currently "
            ...
            else
            {
                ...
                if(offset > 0)
                {
                    System.out.println("Congrats, that is a ladder! " +
                            "You get to " + "climb " + offset + " spaces.");
                }
                // Chute case move backwards
                else if(offset < 0)
                {
                    System.out.println("Sorry, that is a chute! " +
                            "You are sent back " + (0-offset) + " spaces.");
                }
